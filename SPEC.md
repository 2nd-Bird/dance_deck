...
追加仕様6：Timeline 精密調整（UX-TL-SMOOTH-001）
目的
 - iOS写真アプリの動画トリム操作を参照し、ループ範囲の微調整を滑らかに行う

決定事項
 - ループ範囲/左右ハンドルの移動はサブ秒単位で連続的に動くこと
 - 長押しで timeline scale を N倍にする方式で進めてよい（実装は3倍）
 - 操作中の時間表示は必ず `mm:ss:xx` 形式とする
