...
- [ ] (P0) UX-TL-SMOOTH-001 Timeline/Loop操作をiOS Photos準拠で「滑らかさ・追随性・最小幅・見た目」まで仕上げる
  - Status: TODO
  - Impact: コア価値（8-countループ練習）の操作性が低く、精密調整UXも検証不能
  - Expect:
    - ループ窓（range）が00:00:00に吸い付かず、常にドラッグで移動できる
    - 左右ハンドル/範囲移動が指に即追随し、iOS Photosトリム同等に滑らか
    - 4count等の短い範囲でも操作可能（min幅確保のためスケール自動調整）
    - UIの色味/コントラスト/ハンドル/ラベル位置をiOS Photosキャプチャに極力寄せる
  - Actual:
    - ループ窓が00:00:00に吸い付いて動かせないことがある
      → 長押し精密モード（3xスケール）の検証ができない
    - ハンドルはmm:ss:xxで動くが、指より遅れて追随する
      - ドラッグ中に枠が遅れて付いてくる
      - 速く動かすと一瞬大きく動いてから戻り、そこからゆっくり動き出す（意図しない補間/戻り）
    - 4countでは視覚幅が小さく操作困難（hit areaだけでなく表示スケール調整が必要）
    - ループ窓周辺のUIがPhotosの見た目と乖離
  - Repro:
    1) iOS Dev Clientで動画を開く
    2) Timelineのループ窓（range）をドラッグ → 00:00:00に吸い付く/移動できないケースがある
    3) 左右ハンドルをドラッグ → 指から遅れて追随、速い操作で戻り挙動が発生
    4) 4countを選択 → 窓が小さすぎて操作が難しい
  - Logs:
    - (添付予定) TimelineTouchログ、drag時のx/loopStartMillisの推移
  - Notes:
    - 解決方針: minRangePx を定義し、短い範囲では pixelsPerSecond を自動的に上げる（Photosのズーム挙動に寄せる）
    - UI仕様はiOS Photosキャプチャから精密言語化して同等の見え方を目指す

(P0) UX-TL-PHOTOS-STYLE-UI: iOS写真アプリ準拠のトリムUI模倣

- 初期状態は黄色レンジ枠なし（タイムライン帯のみ）
- 左右端のドラッグ開始で黄色レンジ枠を生成
- レンジ外はグレー減光、レンジ内は通常表示
- レンジ端に白い縦バー（つまみ芯）＋内側に黒い山形アイコン（< / >）
- 端ドラッグ中、端直上に mm:ss.xx の角丸グレーラベルを表示（連続更新）
- レンジ中央長押しで精密調整モード：タイムラインズーム（時間軸拡大）で短い範囲も操作可能に
- ドラッグ追随は指と同速（遅れ・ゴム戻り禁止）
